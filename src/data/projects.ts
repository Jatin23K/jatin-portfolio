...
export const projects: Project[] = [
  ...
  },
  {
    ...
    kpiDelta: '100%',
    resumeBullet:
      'Built a full-stack private AI companion ecosystem (Flutter + React + Python) across mobile and desktop, connected over an encrypted Tailscale mesh with shared persistent memory. Kept private because it runs on personal hardware and contains real-world personal data and credentials. Currently in daily personal field-testing.',
    ...
    metrics: [
      ...
      { label: 'Privacy', value: '100% Personal Hardware' },
    ],
  ...
  },
  {
    ...
    title: 'LaunchMintAI',
    oneLineOutcome:
      'Forensic startup intelligence engine combining an XGBoost survival classifier (0.9249 ROC-AUC on 189k startups), 10,000-run NumPy Monte Carlo financial simulations (<35ms), and Aspect-Based Sentiment NLP to eliminate founder confirmation bias.',
    primaryKpi: 'XGBoost ROC-AUC',
    kpiDelta: '0.9249',
    resumeBullet:
      'Engineered a multi-disciplinary Applied DS platform for startup validation — trained an XGBoost survival classifier (0.9249 ROC-AUC, 0.7630 PR-AUC on 189,970 startups) with in-memory SHAP explainability, a 10k-iteration vectorized NumPy Monte Carlo SDE engine (<35ms), and an Aspect-Based Sentiment NLP layer evaluated via a 30-prompt golden RAG Triad benchmark (97.8% Groundedness, 0.0% Hallucinations).',
    ...
    solution:
      'Coupled an XGBoost survival model (0.9249 ROC-AUC) trained on 189,970 startups with SHAP explainability, a 10,000-run NumPy Monte Carlo financial simulation engine, Aspect-Based Sentiment NLP for competitor vulnerability scoring, and a 3-tier domain waterfall search with deterministic regex verification.',
    impact: 'Replaced hallucinated market forecasts with empirical data: 0.9249 ROC-AUC survival scoring, <35ms Monte Carlo cash insolvency curves, and +35% groundedness uplift across a 30-prompt golden evaluation benchmark.',
    tech: ['XGBoost', 'SHAP TreeExplainer', 'NumPy Vectorization', 'Monte Carlo SDEs', 'VADER NLP', 'FastAPI', 'ChromaDB', 'Python'],
    status: 'shipped',
    metrics: [
      { label: 'XGBoost ROC-AUC', value: '0.9249 · (189k Startups)' },
      { label: 'PR-AUC (Imbalance)', value: '0.7630 · (9.11:1 Ratio)' },
      { label: 'Monte Carlo Latency', value: '< 35ms · (10k SDEs)' },
      { label: 'RAG Triad Groundedness', value: '97.8% · (0% Hallucinations)' },
